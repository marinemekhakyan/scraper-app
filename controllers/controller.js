...
module.exports = function(app) {
    app.get("/", function (req, res) {
        request.get("https://www.latimes.com/travel", function(error, response, body) {
            ...
            Promise.each($(".ListI-items").get(), function(newArticles) {
                console.log(newArticles);
                var result = {};
                result.title = $(newArticles).find(".PromoMedium-title").text().trim();
                result.summary = $(newArticles).find(".PromoMedium-content").text().trim();
                result.link = $(newArticles).find("a").attr("href");
                result.image = $(newArticles).find("PromoMedium-media").attr("data-src");
